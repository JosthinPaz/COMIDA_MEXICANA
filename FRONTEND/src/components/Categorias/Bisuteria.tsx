...
const Bisuteria: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchProductos() {
      try {
        const res = await fetch(`${API}/productos/categoria/bisuteria/rich`);
    ...
    <div>
      ...
      <div style={{ maxWidth: 1200, margin: "36px auto", padding: 20 }}>
        <div style={{ background: "#fda792", borderRadius: 20, padding: 24 }}>
          <h1 style={{ margin: 0, fontSize: 28 }}>Bisutería</h1>
        </div>

        <div className="contenedor" style={{ marginTop: 20 }}>
          <div className="menu-lateral" style={{ minWidth: 200 }}>
            <ul>
              <li style={{ fontWeight: 600, color: "#56b47b" }}>Bisutería</li>
            </ul>
          </div>
