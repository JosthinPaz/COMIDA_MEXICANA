    ...
    try {
      if (type === "anio") {
        ...
        const promesas = MESES.map((_, index) => 
          axios.get<{ TotalVentasMensuales?: number }[]>(`${API}/ventas?anio=${anio}&mes=${index + 1}`)
            .then(res => {
        ...
        const res = await axios.get<{ TotalVentasMensuales?: number }[]>(`${API}/ventas${params}`);
        const data = res.data[0] as { TotalVentasMensuales?: number };
        setVentasData([{
          mes: mes || 1,
        ...
        const res = await axios.get<{ TotalVentasDiarias?: number }[]>(`${API}/ventas${params}`);
